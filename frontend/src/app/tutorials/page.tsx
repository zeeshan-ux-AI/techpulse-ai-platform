...
export default async function TutorialsPage() {
  let posts: Post[] = [];
  try {
    const res = await api.getPosts({ type: 'TUTORIAL', limit: '20' });
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10">
      <div className="border-b border-slateDark-800 pb-6 space-y-2">
        ...
        <h1 className="text-4xl font-black text-white">Developer AI Tutorials</h1>
        <p className="text-slate-300 text-sm">Step-by-step technical guides, code snippets, and architecture blueprints.</p>
      </div>
        ...
        {posts.map(post => (
          <article key={post.id} className="p-5 rounded-2xl border border-slateDark-800 bg-slateDark-950/60 hover:border-slateDark-700 transition-all flex flex-col justify-between space-y-4 group">
            ...
            <div className="pt-3 border-t border-slateDark-850 flex items-center justify-between text-xs text-slate-400">
              <span className="font-semibold text-slate-300">{post.author?.name || 'Dr. Elena Rostova'}</span>
